...
def extract(dpath, subpath):
    ...
    for i, all_steps in enumerate(all_steps_per_key):
        assert len(
            set(all_steps)) == 1, "For scalar {} the step numbering or count doesn't match. Step count for all runs: {}".format(
...
def aggregate_to_summary(dpath, aggregation_ops, extracts_per_subpath):
    for op in aggregation_ops:
        for subpath, all_per_key in extracts_per_subpath.items():
            ...
            aggregations_per_key = {key: (steps, wall_times, op(values, axis=0)) for key, (steps, wall_times, values) in
                                    all_per_key.items()}
...
def main(path_arg: str = os.getcwd(), subpaths_arg: list[str] = ['.'], output_arg: str = "summary"):
    path = Path(path_arg)
    if not path.exists():
        raise argparse.ArgumentTypeError("Parameter {} is not a valid path".format(path))

    subpaths = [path / dname / subpath for subpath in subpaths_arg for dname in os.listdir(path) if
                dname != FOLDER_NAME]
    ...
    if output_arg not in ['summary', 'csv']:
        raise argparse.ArgumentTypeError("Parameter {} is not summary or csv".format(output_arg))

    aggregate(path, output_arg, subpaths_arg)
...
if __name__ == '__main__':
    ...
    main(args.path, args.subpaths, args.output)
